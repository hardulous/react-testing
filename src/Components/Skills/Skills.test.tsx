...
describe("Skills", () => {
  ...
  test("Start learning button is not rendered", () => {
    ...
    const startLearningBtn = screen.queryByRole("button", {
      name: "Start learning",
    }); // Not the return can be either HTMLElement || nulll
  ...
  test("Start learning button is eventually displayed", async () => {
    ...
    const startLearningBtn = await screen.findByRole(
      ...
      { name: "Start learning" },
      { timeout: 2000 }
    ); // Here now wait for sometime (default 1000ms) before find the element inside DOM. Now the timeout is 2000ms means findBy* will wait for 2000ms before finding the element in DOM.
